function widget:GetInfo()
	return {
		name = "Top Bar supplyable BP clean", 
...
local function updateResbarText(res)
	...
	if res ~= 'BP' or drawBPBar == true then
		dlistResbar[res][3] = glCreateList(function() 
			font2:Begin()
			-- Text: pull
			if res ~= 'BP' then --if not (res == 'BP' and proMode == false) or drawBPBar == true then -- for bp bar only
...
local function updateResbar(res)  --decides where and what is drawn
			...
			for i = 1, 1 do
				if drawBPIndicators == true and BP[10] ~= nil and BP[11] ~= nil then 
					...
					if res == 'BP' then
						...
						local sizeMultiplyerM = 1
						if playerStallingMetal == 1 then
							sizeMultiplyerM = 2
						end
						local sizeMultiplyerE = 1
						if playerStallingEnergy == 1 then
							sizeMultiplyerE = 2
						end
						...
						glTexRect(math_floor(barArea[1] + (indicatorPosM * barWidth) - (texWidth * sizeMultiplyerM/ 2)), math_floor(barArea[2] - sliderHeightAdd * sizeMultiplyerM), math_floor(barArea[1] + (indicatorPosM * barWidth) + (texWidth / 2)), math_floor((barArea[2] + barArea[4]) / 2 ) - 1)
						glTexture(false)


						glColor(1, 1, 0.6, 1)
						glTexture(":lr" .. texWidth .. "," .. texHeight .. ":LuaUI/Widgets/topbar/triangle.png") 
						glTexRect(math_floor(barArea[1] + (indicatorPosE * barWidth) - (texWidth * sizeMultiplyerE / 2)), math_floor(barArea[4] + sliderHeightAdd - sizeMultiplyerE), math_floor(barArea[1] + (indicatorPosE * barWidth) + (texWidth / 2)), math_floor((barArea[2] + barArea[4]) / 2 ) + 1)
						glTexture(false)
						Log("sliders end" ..i)
					end
				end
	...
	if WG['tooltip'] ~= nil and conversionIndicatorArea then  -- tool tips 
		...
		if res == 'BP' and drawBPBar == true then -- for bp bar only XXX
