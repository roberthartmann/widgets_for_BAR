function widget:GetInfo()
	return {
		name = "Top Bar with Buildpower 0.4.009",
...
cacheDataBase['usedBP_instant'] = 0 																				-- BeHe
cacheDataBase['totalWantedBP'] = 0
cacheDataBase['totalWantedM'] = 0
cacheDataBase['totalWantedE'] = 0																					-- BeHe
	...
	if WG['tooltip'] ~= nil and conversionIndicatorArea then
		...
		if res == 'BP' and config.drawBPBar == true then -- for bp bar only
			...
			if config.debugTooltip then
				...
				bpTooltipText = bpTooltipText
			 		.."\n\nDEBUG:"
					.. float_to_s(avgTotalUsedBP) .. " BP used (smoothed), " .. float_to_s(BP['usedBP_instant']) .. " BP non-stalled ".. float_to_s(BP['totalWantedBP']) .. " if no stall \n"
...
local function updateBPValues()
	if config.drawBPBar then
		...
		BP['totalWantedBP'] = cacheDataBase['totalWantedBP']
		BP['totalWantedM'] = cacheDataBase['totalWantedM']
		BP['totalWantedE'] = cacheDataBase['totalWantedE']
...
local function calculateAverages()
	...
	if BP['usedBP_instant'] >= 1 then
		...
		BP['metalExpensePerBP'] = BP['totalWantedM'] / BP['totalWantedBP']
		BP['energyExpensePerBP'] = BP['totalWantedE'] / BP['totalWantedBP']
...
function widget:GameFrame(n)
	...
	if config.drawBPBar then -- calculations for the exact metal and energy draw value
		...
		local totalReservedBP = 0 -- how much BP is not idle
		
		local totaUsedBP = 0 -- how much BP is actively building
		local totalUsedM = 0
		local totalUsedE = 0

		local totalWantedBP = 0 -- how much BP is actively trying to build
		local totalWantedM = 0
		local totalWantedE = 0
		...
		for i = 1, unitTracking.unitsPerFrame do -- use builderCoroutine, to work though builders
			...
			if not spValidUnitID(unitID) then
				...
				local unitBP, unitIsBuilt, unitDefID, unitTeamID = currentlyInspectedBuilder.buildSpeed, currentlyInspectedBuilder.isBuilt, currentlyInspectedBuilder.unitDefID, currentlyInspectedBuilder.unitTeamID
				...
				if numCommands == 0 then
					...
					if unitType == "nano" and numCommands == 1 and commands[1].id == 16 then
						...
						if builtUnitDefID then
							local _, unitUsedM, _, unitUsedE = spGetUnitResources(unitID)
							totalUsedM = totalUsedM + unitUsedM
							totalUsedE = totalUsedE + unitUsedE

							unitUsedBP = unitUsedM / unitCostData[builtUnitDefID].MperBP -- everything costs at least 1 metal
							...
							local unitBPSupportedByM = unitBP 
							local unitBPSupportedByE = unitBP
							if unitWantedM > 0 then
								unitBPSupportedByM = unitUsedM / unitWantedM *unitBP
							end
							if unitWantedE > 0 then
								unitBPSupportedByE = unitUsedE / unitWantedE *unitBP
							end
							totaUsedBP = totaUsedBP + math_min(unitBPSupportedByM, unitBPSupportedByE)  -- ROBERT BP at the right place
		...
		for unitID, unitReservedBP in pairs(unitsReservedBP) do
			totalReservedBP = totalReservedBP + unitReservedBP
		end
		cacheDataBase['reservedBP_instant'] = cacheDataBase['reservedBP_instant'] + totalReservedBP
		...
		cacheDataBase['usedBPMetalExpense'] = cacheDataBase['usedBPMetalExpense'] + totalUsedM
		cacheDataBase['usedBPEnergyExpense'] = cacheDataBase['usedBPEnergyExpense'] + totalUsedE
