...
local absPath = "LuaUI/Widgets/topbar/"
local png = {
	BP = absPath .. "BP.png",
	con = absPath .. "idle-con.mp3",
	fac = absPath .. "idle-fac.mp3",
}
		...
		if not dlistResValues[res][currentResValue[res]] then
            if res =='BP' then
                ...
                if incomeMetal and incomeMetal ~= 0 then
                    currentResValue[res] = math_floor((r[res][2] - currentResValue[res]) / incomeMetal)
                else
    ...
    if (myTeamID == unitTeam) then
        if unitDef and unitDef.buildSpeed and unitDef.buildSpeed > 0 then
            ...
            if unitDef.isFactory == true then
                Log1("isFactory = 1" ..tostring(unitDef.canAssist) ..tostring(unitDef.name))
                isFactory = 1
            end
            if isFactory == 0 then
                builderUnits[unitID] = unitDef.buildSpeed
                local currentBS = unitDef.buildSpeed
                Log1("was a builder so I:    InitUnit(unitID, unitDefID)" ..tostring(unitID) .."  " ..tostring(unitDefID) .."  " ..tostring(currentBS))
            elseif isFactory == 1 and includeFactories ==true then
                builderUnits[unitID] = unitDef.buildSpeed
                local currentBS = unitDef.buildSpeed
                Log1("includeFactories ==true:        InitUnit(unitID, unitDefID)" ..tostring(unitID) .."  " ..tostring(unitDefID) .."  " ..tostring(currentBS))
            end
        end
    end
end
