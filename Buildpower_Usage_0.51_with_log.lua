...
local function updateUI()
	...
	dlistCU = glCreateList(function()
        ...
        Log1("metalCostOfUsedBuildPower " ..tostring(metalCostOfUsedBuildPower))
        Log1("totalMetalCostOfBuilders " ..tostring(totalMetalCostOfBuilders))
        Log1("metalCostUnusedBuilders " ..tostring(metalCostUnusedBuilders))
        ...
        local fillActiveWidth = barWidth * (totalReservedBPPercentage / 100)  -- % of filling
...
function widget:GameFrame(n)
    if n % 6 == 0 then
        ...
        for unitID, current_unit_BP in pairs(builderUnits) do
            Log1("unitID " ..tostring(unitID))
            totalBP = totalBP + current_unit_BP
            Log1("totalBP " ..tostring(totalBP))
            local unitDefID = Spring.GetUnitDefID(unitID)
            Log1("unitDefID " ..tostring(unitDefID))
            local current_unit_metalCost = UnitDefs[unitDefID].metalCost
            Log1("current_unit_metalCost " ..tostring(current_unit_metalCost))
            ...
            Log1("foundActivity " ..tostring(foundActivity))
            ...
            if foundActivity == true or Spring.GetUnitIsBuilding(unitID)  then -- foundBuildingWish == true
                ...
                Log1("totalReservedBP " ..tostring(totalReservedBP))
                ...
                if currently_used_BP and currently_used_BP > 0 then
                    ...
                    Log1("totally_used_BP " ..tostring(totally_used_BP))
                    metalCostOfUsedBuildPower = metalCostOfUsedBuildPower + current_unit_metalCost * currently_used_BP / current_unit_BP --NEW
                    Log1("metalCostOfUsedBuildPower " ..tostring(metalCostOfUsedBuildPower).. "metalCostOfUsedBuildPower " ..tostring(metalCostOfUsedBuildPower) .. "current_unit_metalCost " ..tostring(current_unit_metalCost).. "currently_used_BP " ..tostring(currently_used_BP).. "current_unit_BP " ..tostring(current_unit_BP))
        ...
        if totalBP > 0 then
            ...
            Log1("totalReservedBPPercentage " ..tostring(totalReservedBPPercentage))
        end
