...
--cacheDataBase[1] ^= cacheDataBase['emtpy1']
--cacheDataBase[2] ^= cacheDataBase['emtpy2'] because BP[2] = BP['totalMetalCostOfBuilders'] gets calculated once
--cacheDataBase[3] ^= cacheDataBase['cacheTotalReservedBP'] 
--cacheDataBase[4] ^= cacheDataBase['emtpy4'] because BP[4] = BP['totalAvailableBP'] gets calculated once				-- BeHe
--cacheDataBase[5] ^= cacheDataBase['cacheTotallyUsedBP']   															-- BeHe
--cacheDataBase[6] ^= cacheDataBase['cacheTotalStallingM']  															-- BeHe
--cacheDataBase[7] ^= cacheDataBase['cacheTotalStallingE']  															-- BeHe
...
local function checkPriority(unitID) -- needed for exact calculations															-- BeHe
...
local function RectQuad(px, py, sx, sy, offset)  -- Rumble
...
local function ChamferedRectQuad(px, py, sx, sy, chamferSize)  -- Rumble
    gl.BeginEnd(GL.QUADS, function()
		-- Bottom chamfers
		gl.Vertex(px + chamferSize, py)
...
local function DrawPartialRing(cx, cy, r, startAngle, endAngle, segments, thickness, color)
    ...
    -- Funktion zum Zeichnen einer Kappe
    local function drawCircleCap(radius, thickness, angle, flip)
        ...
        gl.BeginEnd(GL.TRIANGLE_FAN, function()
            -- Mittelpunkt des Halbkreises (Kappe)
            gl.Vertex(capCenterX, capCenterY)
            for i = 0, segments do
    ...
    -- Zeichnen der Kappen an den Enden des Rings
    drawCircleCap(r, thickness, startAngle, false)  -- Kappe am Startwinkel
    drawCircleCap(r, thickness, endAngle, true)    -- Kappe am Endwinkel, gespiegelt
end
...
function widget:Initialize()
	...
	InitAllUnits()
	for unitDefID, unitDef in pairs(UnitDefs) do -- fill unitCostData this is needed for exact calculations						-- BeHe will get rid of most but metal cost
