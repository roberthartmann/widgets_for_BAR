...
function Log1(Message)  --Logs almost everything
...
local currentResValue = { metal = 1000, energy = 1000, BP = 1000 }
local currentStorageValue = { metal = -1, energy = -1, BP = 500 }


local spaceholder = {800, 2000, 300, 400, 5}
local spaceholder2 = {802, 2002, 302, 402, 52}
...
local function updateResbarText(res)
	...
	if currentStorageValue[res] ~= r[res][2] then
		...
		dlistResbar[res][6] = glCreateList(function()
            ...
            elseif res == 'BP' then
                font2:SetTextColor(0.57, 1.0, 0.45, 1)
			end
...
local function updateResbar(res)
	...
	dlistResbar[res][1] = glCreateList(function()
		...
		elseif res == 'energy' then
            ...
            glColor(0.5, 1, 0.5, 1)
			glTexture(":lr" .. texSize .. "," .. texSize .. ":LuaUI/Images/energy.png")--xxx use BP picture
		end
...
local function drawResbarValues(res, updateText)
    -- xxx
    --r['BP'] = spaceholder xxx space holder
	...
	if not dlistResValuesBar[res][valueWidth] then
		dlistResValuesBar[res][valueWidth] = glCreateList(function()
			...
			elseif res == 'energy' then
				...
				color1 = { 0.2, 0.65, 0, 1 }
				color2 = { 0.6, 0.75, 0, 1 }
				glowAlpha = 0.035 + (0.08 * math_min(1, cappedCurRes / r[res][2] * 40))
			end
...
function widget:Update(dt)
    ...
    if gameFrame % 6 == 0 then
        ...
        BP[3] = totally_used_BP
        BP[4] = totalBP
        BP[5] = totalReservedBP
...
function InitUnit(unitID, unitDefID, unitTeam)
    ...
    if (myTeamID == unitTeam) then
        if unitDef and unitDef.buildSpeed and unitDef.buildSpeed > 0 then
            ...
            Log("InitUnit(unitID, unitDefID)" ..tostring(unitID) .."  " ..tostring(unitDefID) .."  " ..tostring(currentBS))
        end
    end
end
