...
function widget:GetInfo()
    return {
        name = "Top Bar with Buildpower PRE BEHE",
...
local function updateResbarText(res)
    ...
    if currentStorageValue[res] ~= r[res][2] then
        ...
        dlistResbar[res][6] = glCreateList(function()
            ...
            if res == 'BP' and config.proMode and config.drawBPBar then
                -- total buildpower
    ...
    if res ~= 'BP' or config.drawBPBar then
        dlistResbar[res][3] = glCreateList(function() 
            ...
            elseif res == 'BP' and config.proMode then
                -- used buildpower
            ...
            elseif res == 'BP' and config.proMode then
                -- reserved buildpower (including used)
...
function widget:GameFrame(n)
    ...
    Log("cache")
    local gameFrameFreq = 2 -- TODO: get this to 1 --- Do we need update faster than the gadget does? I guess not.
    ...
    if config.drawBPBar and ((gameFrame % gameFrameFreq == 0) or nowChecking > 0) then
        ...
        for unitID, unitData in pairs (trackedBuilders) do --calculation of exact pull
            ...
            if nowChecking >= trackPosBase then -- begin at trackedPos with calcs --- Do we need to split the tracking if we don't calculate as much? 
                ...
                if not unitIsBuilt then
                    ...
                    if isDead ~= false then -- unit is dead _or_ nonexistent  --- Redundant? Checking if unit exists, here
                        UntrackUnit(unitID, unitDefID, unitTeamID)
                    else
                        -- Units still being built count as reserved.  --- is that so? maybe this causes the bug.
                    ...
                    if not unitExists then   --- Redundant? Checking if unit exists, here why can't we just check isDead once before other functions are called?
                        UntrackUnit(unitID)
                    elseif not foundActivity then
                        ...
                        if mayBeBuilding and not builtUnitDefID then  --- we are finding the commands anyways. Can't we ask for: is using BP? then used BP (we don't even need to track that. Behes gives infos); esleif guarding idle? then idle  elseif command found then reserved; else idle
                        ...
                        BEHE
--> List of Build (ID of BP and want BP)

--Look at
---> List -> just Ignore these, because they are in BEHE
--Look at Rest 
--Max reseved
--Save state reserved,  busy or idle
--If guard then look at guarded ID 
    --If guarded ID  not in buider ID OR full health then 
        --reserved
    --Else go through guarded Ids of builders
        --  create a list of Ids that where went trough. 
        --	If a id appears twice, then loop
            --	state Idle for all those IDs
        --  else there is a end of guarded row, then ->
            --  look at the state of this last, guarded Builder
            --  Save that state to all the builder Ids in the row
        --  end
    --end
--end
                        ...
                        if builtUnitDefID then --- Behe does all that for us
        ...
        if config.guardingIdleBuilderCountsAsIdle then --- make a list of idle and guarding builders using findBPCommand -> compare them -> if guarding an Buider then go through the guarding IDs until you found the last one. if the last one (e.g. 15 deep because of endless loops) is idle then idle all of them else reserved
        ...
        cacheDataBase[5] = cacheDataBase[5] + cacheTotallyUsedBP  --- Behe
        cacheDataBase[6] = cacheDataBase[6] + cacheTotalStallingM --- Behe
        cacheDataBase[7] = cacheDataBase[7] + cacheTotalStallingE --- Behe
        ...
        cacheDataBase['usedBPMetalExpense'] = cacheDataBase['usedBPMetalExpense'] + usedBPMetalExpense --- Behe
        cacheDataBase['usedBPEnergyExpense'] = cacheDataBase['usedBPEnergyExpense'] + usedBPEnergyExpense --- Behe
        cacheDataBase['usedBPExceptStalled'] = cacheDataBase['usedBPExceptStalled'] + nonStalledBuildingBP --- Behe
        cacheDataBase['usedBPIfNoStall'] = cacheDataBase['usedBPIfNoStall'] + buildingBP   --- Behe

        if config.drawBPWindRangeIndicators then --- why do we need real wind strength?
    ...
    if trackPosBase >= trackedNum then --- Do we need to split the tracking if we don't calculate as much? 
        trackPosBase = 0
        if config.drawBPBar then --this section will smooth the values so that factories that finish units won't have too much of an impact
            BP['reservedBP_instant'] = cacheDataBase[3]  --- BEHE + easier calcs
            BP['usedBP_instant'] = cacheDataBase[5]   --- BEHE
            BP['metalIncome'] = cacheDataBase['metalIncome']
            BP['energyIncome'] = cacheDataBase['energyIncome']
            BP['realWindStrength_instant'] = cacheDataBase['realWindStrength']  --- why realWindSprength? 
            BP['metalExpense'] = cacheDataBase['metalExpense']
            BP['energyExpense'] = cacheDataBase['energyExpense']
            BP['usedBPMetalExpense'] = cacheDataBase['usedBPMetalExpense']  --- BEHE
            BP['usedBPEnergyExpense'] = cacheDataBase['usedBPEnergyExpense']  --- BEHE
            BP['usedBPExceptStalled'] = cacheDataBase['usedBPExceptStalled']  --- BEHE
            BP['usedBPIfNoStall'] = cacheDataBase['usedBPIfNoStall']  --- BEHE
            ...
            if config.drawBPWindRangeIndicators then  --- why realwind strength?
            ...
            if BP['usedBP_instant'] >= 1 then
                ...
                local m_per_bp = 0
                if BP['metalExpensePerBP'] ~= nil then  --- Behe
                    m_per_bp = BP['metalExpensePerBP']
                end
                local e_per_bp = 0
                if BP['energyExpensePerBP'] ~= nil then  --- Behe
                    e_per_bp = BP['energyExpensePerBP']
                end
                -- How much metal and energy are we spending _not_ due to builders?
                local metalExpenseMinusBuilders_instant  = BP['metalExpense'] - BP['usedBP_instant'] * m_per_bp --- metal expense - Behe
                local energyExpenseMinusBuilders_instant = BP['energyExpense'] - BP['usedBP_instant'] * e_per_bp --- energy expense - Behe
                ...
                BP['metalExpensePerBP'] = BP['usedBPMetalExpense'] / BP['usedBP_instant']  --- Behe
                BP['energyExpensePerBP'] = BP['usedBPEnergyExpense'] / BP['usedBP_instant']  --- Behe
                BP['metalExpenseIfAllBPUsed'] = metalExpenseMinusBuilders + BP['metalExpensePerBP'] * totalBP   --- Behe
                BP['energyExpenseIfAllBPUsed'] = energyExpenseMinusBuilders + BP['energyExpensePerBP'] * totalBP  --- Behe
                ...
                if BP['energyExpensePerBP'] > 0 then
                    ...
                    if config.drawBPWindRangeIndicators then
                        eSupportedBP_minWind = (BP['energyIncomeNoWind'] - energyExpenseMinusBuilders) / BP['energyExpensePerBP'] --- why no minEPerWindGen?
            ...
            BP['mSliderPosition'] = addSampleAndGetWeightedAverage(BP['history_mSliderPosition'], bpRatioSupportedByMIncome, BP['usedBP_instant'])
            BP['eSliderPosition'] = addSampleAndGetWeightedAverage(BP['history_eSliderPosition'], bpRatioSupportedByEIncome, BP['usedBP_instant'])

            updateResbar('BP')
            cacheDataBase[3] = 0  --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase[5] = 0  --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase['metalIncome'] = 0  --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase['energyIncome'] = 0 --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase['metalExpense'] = 0 --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase['energyExpense'] = 0 --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase['realWindStrength'] = 0 --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase['usedBPMetalExpense'] = 0 --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase['usedBPEnergyExpense'] = 0 --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase['usedBPExceptStalled'] = 0 --- Do we need to split the tracking if we don't calculate as much? 
            cacheDataBase['usedBPIfNoStall'] = 0 --- Do we need to split the tracking if we don't calculate as much? 

        end
        totalStallingM = cacheDataBase[6] --- Do we need to split the tracking if we don't calculate as much? 
        totalStallingE = cacheDataBase[7] --- Do we need to split the tracking if we don't calculate as much? 
        cacheDataBase[6] = 0 --- Do we need to split the tracking if we don't calculate as much? 
        cacheDataBase[7] = 0 --- Do we need to split the tracking if we don't calculate as much? 
        nowChecking = 0 --- Do we need to split the tracking if we don't calculate as much? 
    end
...
local function updateAllyTeamOverflowing()
    ...
    for i, teamID in pairs(teams) do
        ...
        if teamID == myTeamID then
            ...
            if res ~= 'BP' or config.drawBPBar == true then  --- try to delete this
...
local sec = 0  --- update for ResBar  hover
local sec2 = 0  --- update for text on ResBar 
local secComCount = 0
...
function widget:Update(dt)
    ...
    local mx, my = spGetMouseState()   --- check the Mouse Pos as fast as possible
    ...
    sec = sec + dt
    if sec > 0.1 then  --- why 0.1 and not 0.033 -> still to much calculation?
        Log("sec > 0.1")
        sec = 0
        r = { metal = { spGetTeamResources(myTeamID, 'metal') }, energy = { spGetTeamResources(myTeamID, 'energy') }, BP = BP }  --- so oft nachfragen?
...
function shutdown()
    ...
    if WG['tooltip'] ~= nil then
        ...
        WG['tooltip'].RemoveTooltip(res .. '_income')
        WG['tooltip'].RemoveTooltip(res .. '_storage')
        WG['tooltip'].RemoveTooltip(res .. '_current') --- xxx hier brauche ich auch meine Tooltips
    end
...
function TrackUnit(unitID, unitDefID, unitTeam, isBuilt) -- needed for exact calculations
    ...
    if (myTeamID == unitTeam) and unitDef then
        if unitDef.buildSpeed and unitDef.buildSpeed > 0 and unitDef.canAssist then
            ...
            if unitDef.isFactory == false or config.includeFactories then  --- only track factories if it is desired 
                trackedNum = trackedNum + 1
                local unitDefID = Spring.GetUnitDefID(unitID) --handling metal cost
                local currentUnitMetalCost = 100  ---   IS TIME YOU ARE BEHIND NEEDED?
                ...
                if not trackedBuilders[unitID] then   ---   IS TIME YOU ARE BEHIND NEEDED?  AND IS TOTAL BP NEEDED?
        ...
        elseif isBuilt and (unitDef.name == "armwin" or unitDef.name == "corwin") then -- wind generator  --- only if show wind indicator? what if there are BIG Winds? THIS IS THE ONLY THING I NEED? + unitID
...
function UntrackUnit(unitID, unitDefID, unitTeam) -- needed for exact calculations  --- LOOK AT THE trackUnit function
    local unitDef = UnitDefs[unitDefID]
    if (myTeamID == unitTeam) and unitDef then
        if unitDef.buildSpeed and unitDef.buildSpeed > 0 and unitDef.canAssist then
            -- is factory is needed for bp bar only (switch metal cost on/off)
